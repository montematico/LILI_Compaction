function plot_compaction_results(dataFile)
    ...
    %% Design Robustness Histogram
    unique_masses = unique(M_ROV_grid);
    success_rate = zeros(size(unique_masses));
    
    for i = 1:length(unique_masses)
        mask = (M_ROV_grid == unique_masses(i));
        success_rate(i) = (sum(Valid_mask(mask)) / sum(mask)) * 100;
    end
    
    figure('Name', 'Configuration Success Rate');
    bar(unique_masses, success_rate);
    ...
    %% Graphics and Plots (Relocated from ChenSweep.m)
